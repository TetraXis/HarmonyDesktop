...
sf::IpAddress server_ip("25.41.207.37");
unsigned short server_port(443);

Server* global_server = new Server;
...
void server_test(sf::IpAddress adress = server_ip, unsigned short port = server_port)
{
	sf::UdpSocket testSocket;
	for (int i = 0; i < test.size(); i++)
		testSocket.send(test[i].c_str(), test[i].size(), server_ip, server_port);
}
...
void ServerReceiveProc(Server* server)
{
	...
	while (server->running)
	{
		...
		{
			std::cout << "Received:\n";
			std::cout << std::string(receivedData, receivedData + receivedSize) << '\n';

			decodedData = decodeData(receivedData, receivedSize);

			std::cout << "==> " << decodedData[0] << '\n';

			if (decodedData[0] == "server_join")
			{
				std::cout << "++> " << receivedIp.toString() << '\n';

				harmony::global_terminal.execute(std::wstring(receivedData, receivedData + receivedSize) + L" " + str_to_wstr(receivedIp.toString()));
				continue;
			}
